...
static bool NRF5_ESB_sendMessage(uint8_t recipient, const void *buf, uint8_t len, const bool noACK)
{
	...
	while ((tx_retries > 0) && (ack_received == false)) {
		...
		if (tx_retries > 0) {
			...
			NRF5_RADIO_TIMER->CC[3] = NRF5_ESB_ARD + NRF5_ESB_MAX_PACKET_TIME + (tx_pid << NRF5_ESB_byte_time());
		} else {
			// Wait for ACK with a litte jitter
			NRF5_RADIO_TIMER->CC[3] = NRF5_ESB_ARD + NRF5_ESB_ACK_WAIT + NRF5_ESB_MAX_PACKET_TIME + (tx_pid << NRF5_ESB_byte_time());;
		}
...
void print_radio_stats()
{
	NRF5_RADIO_DEBUG(PSTR("NRF5:INTCTR:ADDRM=%" PRIu32 ",END=%" PRIu32 ",DISABLED=%" PRIu32 ",TMRC3=%" PRIu32 "\n"),

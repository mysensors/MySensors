...
#include "Radio.h"
#include "Radio_ESB.h"
#include "core/MyHwNRF5.h"
#include "drivers/CircularBuffer/CircularBuffer.h"
#include <stdio.h>

// Show interrupt reason, when enabled timing is violated!
//#define NRF5_ESB_DEBUG_INT_TYPE
//#define NRF5_ESB_DEBUG_INT_RX_DATA
//#define NRF5_ESB_DEBUG_INT_RX_ACK
//#define NRF5_ESB_DEBUG_INT_END_TX
//#define NRF5_ESB_DEBUG_INT_TX_END
...
{
	NRF5_RADIO_DEBUG(PSTR("NRF5:INIT,Initialize ESB mode\n"));
	...
	NRF5_RADIO_DEBUG(PSTR("NRF5:INIT,BASE0=%08x,PF0=%08x,RXAD=%08x\n"),
	                 NRF_RADIO->BASE0, NRF_RADIO->PREFIX0,
	...
	// Enable interrupt
	NRF5_RADIO_TIMER->INTENSET = TIMER_INTENSET_COMPARE0_Enabled
	                             << TIMER_INTENSET_COMPARE0_Pos;
	NRF5_RADIO_TIMER->INTENSET = TIMER_INTENSET_COMPARE1_Enabled
	...
	NRF_PPI->CH[NRF5_RADIO_PPI_TIMER_RADIO_DISABLE].EEP =
	    (uint32_t)&NRF5_RADIO_TIMER->EVENTS_COMPARE[0];
	NRF_PPI->CH[NRF5_RADIO_PPI_TIMER_RADIO_DISABLE].TEP =
...
static void NRF5_ESB_powerDown()
{
	...
	// Clear PPI
	NRF_PPI->CHENCLR = NRF5_RADIO_PPI_BITS;
...
static void NRF5_ESB_setNodeAddress(const uint8_t address)
{
	...
	NRF_RADIO->PREFIX0 = (NRF_RADIO->PREFIX0 & NRF5_ESB_NODE_ADDR_MSK) |
	                     reverse_byte(address) << (NRF5_ESB_NODE_ADDR << 5);
	NRF5_RADIO_DEBUG(PSTR("NRF5:SND,PF0=%08x,RXAD=%08x\n"), NRF_RADIO->PREFIX0,
	                 NRF_RADIO->RXADDRESSES);
}

static uint8_t NRF5_ESB_getNodeID()
{
	return reverse_byte((NRF_RADIO->PREFIX0 & NRF5_ESB_NODE_ADDR_MSK) >>
...
static void NRF5_ESB_startListening()
{
	...
	// Enable READY Event for setting the RX buffer
...
static uint8_t NRF5_ESB_readMessage(void *data)
{
	...
	NRF5_RADIO_DEBUG(PSTR("NRF5:RX,LEN=%d,NOACK=%d,PID=%d,"
	                      "RX=%d\n"),
	                 buffer->len, buffer->noack, buffer->pid, buffer->rxmatch);
#endif
...
void NRF5_ESB_starttx()
{
	if (tx_retries > 0) {
		...
		// Disable READY Event for setting the RX buffer
		...
		// Set PPI
		NRF_PPI->CHENSET = NRF5_RADIO_PPI_BITS;
                                 ...
                                 uint8_t len)
{
	...
	tx_retries = ((recipient == BROADCAST_ADDRESS) ? (NRF5_ESB_BC - 1)
	              : (NRF5_ESB_ARC - 1));
#ifdef MY_DEBUG_VERBOSE_NRF5_ESB
	...
	NRF_RADIO->RXADDRESSES = (1 << NRF5_ESB_NODE_ADDR) | (1 << NRF5_ESB_BC_ADDR) |
	                         (1 << NRF5_ESB_TX_ADDR);
	//  NRF_RADIO->RXADDRESSES = 0xffffffffUL;

	NRF5_RADIO_DEBUG(PSTR("NRF5:SND,PF1=%08x,RXAD=%08x\n"), NRF_RADIO->PREFIX1,
	                 NRF_RADIO->RXADDRESSES);
...
#ifdef MY_DEBUG_VERBOSE_NRF5_ESB
	NRF5_RADIO_DEBUG(PSTR("NRF5:SND,END=%d,ACK=%d,RTRY=%d,WAKE=%d\n"),
	                 events_end_tx, ack_received, tx_retries_start - tx_retries,
	                 wakeups);
#endif
...
extern "C" {
	...
	void RADIO_IRQHandler()
	{
...
#ifdef NRF5_ESB_DEBUG_INT_TYPE
			NRF5_RADIO_DEBUG(PSTR("NRF5:INT:BC\n"));
#endif
			...
			if (NRF_RADIO->STATE == RADIO_STATE_STATE_Rx) {
				...
				if ((NRF_RADIO->RXMATCH != NRF5_ESB_NODE_ADDR) || (rx_buffer->noack)) {
					...
					if (NRF_RADIO->RXMATCH == NRF5_ESB_TX_ADDR) {
						...
						NRF5_RADIO_TIMER->CC[0] +=
						    ((rx_buffer->len + 3) << NRF5_ESB_byte_time());
#if __CORTEX_M == 0x04
		...
		/** Ready event is only enabled on RX or RX-ACK mode
		 * Try to get an free rx buffer or disable radio on failures
		 */
		if (NRF_RADIO->EVENTS_READY == 1) {
			NRF_RESET_EVENT(NRF_RADIO->EVENTS_READY);
#ifdef NRF5_ESB_DEBUG_INT_TYPE
			NRF5_RADIO_DEBUG(PSTR("NRF5:INT:RDY\n"));
#endif
			...
			// Prepare outgoing ACK packet
			rx_buffer->len = 0;
			rx_buffer->pid = ack_pid++;
			rx_buffer->noack = 0;
...
#ifdef NRF5_ESB_DEBUG_INT_TYPE
			NRF5_RADIO_DEBUG(PSTR("NRF5:INT:END %d\n"), NRF_RADIO->STATE);
#endif
			...
			if ((NRF_RADIO->STATE == RADIO_STATE_STATE_Rx) or
				...
				if (NRF_RADIO->CRCSTATUS) {
					...
					if (NRF_RADIO->RXMATCH != NRF5_ESB_TX_ADDR) {
						...
						// ACK package, ducplicates are accepted
						ack_received = true;
						// End TX
						NRF5_ESB_endtx();
#ifdef NRF5_ESB_DEBUG_INT_RX_ACK
						NRF5_RADIO_DEBUG(PSTR("NRF5:INT:RX:ACK\n"));
#endif
					}
				} else {
...
#ifdef NRF5_ESB_DEBUG_INT_RX_DATA
				NRF5_RADIO_DEBUG(PSTR("NRF5:INT LEN=%d,CRC=%d,NOACK=%d,PID=%d,"
				                      "RX=%d\n"),
				                 rx_buffer->len, NRF_RADIO->CRCSTATUS, rx_buffer->noack,
				                 rx_buffer->pid, NRF_RADIO->RXMATCH);
#endif
			} else {
#ifdef NRF5_ESB_DEBUG_INT_END_TX
				NRF5_RADIO_DEBUG(PSTR("NRF5:INT:TXEND\n"));
#endif
			}
		}
	}

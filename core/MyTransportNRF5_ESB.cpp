...
#if defined(MY_NRF5_ESB_ENABLE_ENCRYPTION)
  hwReadConfigBlock((void *)_psk, (void *)EEPROM_RF_ENCRYPTION_AES_KEY_ADDRESS,
                    16);
  // set up AES-key
...
uint8_t transportGetAddress(void) { return NRF5_ESB_getNodeID(); }
...
bool transportSend(const uint8_t to, const void *data, const uint8_t len) {
  ...
  return NRF5_ESB_sendMessage(to, _dataenc, finalLength);
#else
  return NRF5_ESB_sendMessage(to, data, len);
#endif
}

bool transportAvailable(void) { return NRF5_ESB_isDataAvailable(); }

bool transportSanityCheck(void) { return NRF5_ESB_sanityCheck(); }
...
void transportPowerDown(void) { NRF5_ESB_powerDown(); }

...
void i2c_eeprom_write_buffer(int deviceaddress, unsigned int eeaddress, byte* data, byte data_len)
{
	// Uses Page Write for 24LC256
	// Allows for 32 byte page boundary
	...
	page_space = int(((eeaddress/32) + 1)*32)-eeaddress;
...
void hwWriteConfigBlock(void* buf, void* adr, size_t length)
{
	...
	int offs = reinterpret_cast<int>(adr);
	i2c_eeprom_write_buffer(I2C_EEP_ADDRESS, offs, src, length);
	//	while (length-- > 0) {

...
// static variables
uint8_t MYS_RF24_base_addr[MY_RF24_ADDR_WIDTH] = { MY_RF24_BASE_RADIO_ID };
uint8_t MYS_RF24_rxaddr_register;
uint8_t MYS_RF24_nodeAddress = AUTO;
...
uint8_t spiMultiByteTransfer(uint8_t cmd, uint8_t* buf, uint8_t len, boolean aReadMode) {
	uint8_t* current = buf;
	csn(LOW);
...
#define readRegister(reg) _readRegister( R_REGISTER | ( REGISTER_MASK & (reg) ) )
uint8_t _readRegister(uint8_t cmd) {
...
void startListening(void) {
	...
	// toggle PRX		
	writeByteRegister(NRF_CONFIG, _CONFIGURATION | _BV(PWR_UP) | _BV(PRIM_RX) );	
	// all RX pipe addresses must be unique, therefore skip if node ID is 0xFF
	if(MYS_RF24_nodeAddress!=AUTO) writeByteRegister(RX_ADDR_P0, MYS_RF24_nodeAddress);
	// start listening
	ce(HIGH);
}
...
void stopListening(void) {
	...
	writeByteRegister(NRF_CONFIG, _CONFIGURATION | _BV(PWR_UP) );
...
uint8_t getNodeID(void) {
	return MYS_RF24_nodeAddress;
}

boolean initializeRF24(void) {
	// Initialize pins
	pinMode(MY_RF24_CE_PIN,OUTPUT);
	pinMode(MY_RF24_CS_PIN,OUTPUT);
	// Initialize SPI
	_SPI.begin();
	// SoftSPI runs at ~2Mhz, no adjustments needed
	...
	MYS_RF24_base_addr[0] = BROADCAST_ADDRESS;
	writeMultiByteRegister(RX_ADDR_P0 + BROADCAST_PIPE, (uint8_t*)&MYS_RF24_base_addr, BROADCAST_PIPE > 1 ? 1 : MY_RF24_ADDR_WIDTH);
	// pipe 0, set full address, later only LSB is updated
	writeMultiByteRegister(RX_ADDR_P0, (uint8_t*)&MYS_RF24_base_addr, MY_RF24_ADDR_WIDTH);
	writeMultiByteRegister(TX_ADDR, (uint8_t*)&MYS_RF24_base_addr, MY_RF24_ADDR_WIDTH);
	// reset FIFO
	flushRX();
	flushTX();
	// reset interrupts

/*pyright (C) 2011 J. Coliz <maniacbug@ymail.com>
...
{
	if (prefix0x)
	MY_SERIALDEVICE.print(F("0x"));

	MY_SERIALDEVICE.print(v >> 4, HEX);
	MY_SERIALDEVICE.print(v & 0x0F, HEX);
}
#endif
...
void RF24::print_status(uint8_t status) const
{
	...
	MY_SERIALDEVICE.print(F(" RX_DR="));   MY_SERIALDEVICE.print((status & _BV(RX_DR))     ? one : zero );
	MY_SERIALDEVICE.print(F(" TX_DS="));   MY_SERIALDEVICE.print((status & _BV(TX_DS))     ? one : zero );
	MY_SERIALDEVICE.print(F(" MAX_RT="));  MY_SERIALDEVICE.print((status & _BV(MAX_RT))    ? one : zero );
	MY_SERIALDEVICE.print(F(" RX_P_NO=")); MY_SERIALDEVICE.print((status >> RX_P_NO) & uint8_t(B111));
	MY_SERIALDEVICE.print(F(" TX_FULL=")); MY_SERIALDEVICE.println((status & _BV(TX_FULL)) ? one : zero );
#else
	(void)status;
#endif
}
...
void RF24::printDetails(void)
{
	...
	MY_SERIALDEVICE.print(F("TX_ADDR\t\t"));     print_address_register(TX_ADDR);

	MY_SERIALDEVICE.print(F("RX_PW_P0-6\t"));    print_byte_register(RX_PW_P0,6);
	MY_SERIALDEVICE.print(F("EN_AA\t\t"));       print_byte_register(EN_AA);
	MY_SERIALDEVICE.print(F("EN_RXADDR\t"));     print_byte_register(EN_RXADDR);
	MY_SERIALDEVICE.print(F("RF_CH\t\t"));       print_byte_register(RF_CH);
	MY_SERIALDEVICE.print(F("RF_SETUP\t"));      print_byte_register(RF_SETUP);
	MY_SERIALDEVICE.print(F("CONFIG\t\t"));      print_byte_register(NRF_CONFIG);
	MY_SERIALDEVICE.print(F("DYNPD/FEATURE\t")); print_byte_register(DYNPD,2);

	MY_SERIALDEVICE.print(F("Data Rate\t"));     MY_SERIALDEVICE.println(PGM_READ_PTR(&rf24_datarate_e_str_P[getDataRate()]));
	MY_SERIALDEVICE.print(F("Model\t\t"));       MY_SERIALDEVICE.println(PGM_READ_PTR(&rf24_model_e_str_P[isPVariant()]));
	MY_SERIALDEVICE.print(F("CRC Length\t"));    MY_SERIALDEVICE.println(PGM_READ_PTR(&rf24_crclength_e_str_P[getCRCLength()]));
	MY_SERIALDEVICE.print(F("PA Power\t"));      MY_SERIALDEVICE.println(PGM_READ_PTR(&rf24_pa_dbm_e_str_P[getPALevel()]));
#endif
}
/****************************************************************************/
...
void RF24::errNotify(){
#ifdef MY_DEBUG
	MY_SERIALDEVICE.println(F("RF24 HARDWARE FAIL"));
#endif
	failureDetected = 1;
}
#endif
...
void RF24::maskIRQ(bool tx, bool fail, bool rx){
	...
	write_register(NRF_CONFIG, ( read_register(NRF_CONFIG) ) | fail << MASK_MAX_RT | tx << MASK_TX_DS | rx << MASK_RX_DR  );
}
